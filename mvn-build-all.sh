...
# 1.7.0_xx : 7
# 1.8.0_xx : 8
declare -i JDK_VERSION=`javac -version 2>&1 | sed 's/javac \([0-9]\).\([0-9]\).*/\2/;'`
...
for target in ${TARGETS_JAVA8_PLUS}; do
    if test -f ${artifactId}/pom.xml ; then
        ...
        if test ${JDK_VERSION} -lt 8 && test -n ${JAVA8_PLUS_HOME} ; then
...
if test ${JDK_VERSION} -lt 8 ; then
    echo "[WARNING] In below modules, skip a compile(and install, deploy) because it need a JDK 8+.";
    for target in ${TARGETS_JAVA8_PLUS}; do
